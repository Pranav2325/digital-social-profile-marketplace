                ...
                <button onClick={()=>handleOpenChat(chat)} key={chat.id} className="w-full p-4 hover:bg-gray-50 transition-colors text-left">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-shrink-0">
                      <img src={chatUser?.image} alt={chat?.chatUser?.name} className="w-12 h-12 rounded-lg objcet-cover" />
                    </div>
                    {/* title,username,lastmessage */}
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-sm truncate ${!chat.isLastMessageRead && chat.lastMessageSenderId !== user?.id? 'text-indigo-600 font-medium':"text-gray-500"}`}>{chat.lastMessage ||"No messsages yet"}</p>
